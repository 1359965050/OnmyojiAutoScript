...
# @author runhey
"""收集配置摘要与关键日志为 zip，在服务端脱敏账号/token/密码/手机号。"""
import json
import platform
import re
import sys
...
def build_diagnostic_zip(config_name: str = '') -> Path:
    ...
    # 只收录已存在的实例，防止 config_name 包含 ../ 造成路径穿越
    ...
    logger.info(f'诊断日志已打包: {zip_path}')
    return zip_path
