...
img_path = r"C:\Users\13599\.gemini\antigravity-ide\brain\22181a7e-5403-4990-b871-d2e327a0e08e\media__1782147396743.png"
...
for res in ocr_results:
    ...
    if slot_idx is not None:
        ...
        if 380 <= cy <= 460:
            # Only extract levels from description lines containing keywords like 升, 提, 增, %
            if any(k in text for k in ["升", "提", "增", "%"]):
                match = re.search(r'(\d+)', text)
                if match:
                    val = int(match.group(1))
                    if 0 <= val <= 100:
                        print(f"DEBUG: Slot {slot_idx} matched '{text}' at cy={cy}, extracted {val}")
