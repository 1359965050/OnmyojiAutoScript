...
- 后端 API 固定运行于 `localhost:7788`，由 `server.py` 启动。
- 当前 FastAPI 仅注册三个路由：`home_app`（首页/菜单）、`script_app`（脚本实例管理）、`tool_app`（工具）。
- `图像服务` 和 `统计服务` 已从前端 OASX 和后端移除，相关文件（`image_router.py`、`stats_router.py`、`stats_service.py` 等）不再存在。
- 前端 OASX 左侧二级菜单当前仅保留：主页、更新器；脚本页保留总览和任务配置列表。
