...
## 1. 项目身份与硬性约束
- **定位**：基于 OAS 官方 dev 分支的**本地单机二开版**，仅供个人学习。
- **网络隔离**：
...
- **活动/副本**：六道之门（预留孔雀国适配）、兵藏秘境（英杰试炼）、八岐大蛇（仅队长/队员/单刷，无野队）、爬塔（ActivityShikigami）。
...
- **六道之门（孔雀国）闭环**：
  - 已打通“选关 → 备战 → 战斗 → 首领结算（自动使用双倍/关窗）”全流程。
  - 混沌之屿自动识别“精英”并挑战，非精英秒退；宁息之屿（商店）强制秒退，不购物。
- **百鬼夜行**：集成 ONNX 追踪器，自动锁定最高稀有度鬼王（SP>SSR>...），遇冻结滤镜自动暂停追踪。
- **结界挂卡**：强制在挂卡前检查并更换满级式神（无可选开关）。

## 4. 性能与启动优化（运维重点）
- **启动加速**：OCR 库（ONNXRuntime）已改造为**惰性加载（Lazy Import）**，若未启用 OCR 功能，启动时不加载 C++ DLL，大幅降低内存。
- **配置缓存**：`MainManager` 已实现内存缓存，反复读取配置耗时从秒级降至毫秒级。
- **无任务处理**：当无任务启用时，不再抛出异常导致 WS 断开，而是返回空任务“Idle”。

## 5. 前端 GUI（OASX）适配须知
- **已隐藏菜单**：侧边栏已移除“工具（Tool）”“统计服务（Stats）”“图像服务（Image）”；日常任务中删除了小猫咪、年兽、花合战等，周常中删除了真蛇、神秘商店。
- **本地化中文化**：八岐大蛇身份选项已强制显示为“队长/队员/单独刷”。
- **组队调度**：配置面板中的 `team_flow` 在中文环境下显示为“组队调度”。

## 6. 开发调试指引（二开必看）
- **新增配置**：请在 `module/config/config_model.py` 中注册 Pydantic 字段，OASX 会自动渲染。
- **界面素材更新**：若游戏 UI 改版，运行 `./dev_tools/assets_extract.py` 重制识别图。
- **注意**：本项目所有 `template.json` 和 `oas1.json` 已清除了已删除模块的配置节点，请勿将旧版完整配置文件直接覆盖，否则会报 `AttributeError`。